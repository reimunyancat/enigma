...
<div class="cfg">
  ...
  <div class="field">
    <label>{$t.reflector}</label>
    <select
      value={$config.reflector}
      on:change={(e) => {
        $config.reflector = e.currentTarget.value as Reflector;
        apply();
      }}
    >
  ...
  <div class="field">
    <label>{$t.ringSetting}</label>
    <input
      class="tri"
      maxlength="3"
      value={$config.rings}
      on:input={(e) => {
        $config.rings = clean(e.currentTarget.value, 3);
        apply();
      }}
    />
  </div>
  <div class="field">
    ...
    <input
      ...
      on:input={(e) => {
        $config.positions = clean(e.currentTarget.value, 3);
        apply();
      }}
    />
  </div>
  <div class="field">
    <label>{$t.plugboard}</label>
    <input
      class="pl"
      value={$config.plugs}
      on:input={(e) => {
        $config.plugs = clean(e.currentTarget.value, 26);
        apply();
      }}
    />
  </div>
