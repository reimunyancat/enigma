<script lang="ts">
  ...
  import { rotorPos, lamp, lastKey, config, press } from "../machine"
  ...
  onMount(() => {
    ...
    function woodTexture(): THREE.CanvasTexture {
      ...
      for (let i = 0; i < 2600; i++) {
        ...
        g.beginPath()
        g.moveTo(0, y)
        g.bezierCurveTo(w * 0.33, y + (Math.random() - 0.5) * 14, w * 0.66, y + (Math.random() - 0.5) * 14, w, y + (Math.random() - 0.5) * 10)
        g.stroke()
      }
    ...
    // 위를 보는(바닥에 눕는) 라벨
    function flatLabel(ch: string, color: string, size: number, tint?: number): THREE.Mesh {
      const m = new THREE.MeshBasicMaterial({ map: letterTex(ch, color), transparent: true, depthWrite: false })
    ...
    function logoTex(): THREE.CanvasTexture {
      ...
      g.lineWidth = 7
      g.beginPath(); g.ellipse(256, 128, 208, 78, 0, 0, Math.PI * 2); g.stroke()
      g.fillStyle = "#ece4d0"
    ...
    // ── 재질 ──
    const woodMat = new THREE.MeshStandardMaterial({ map: woodMap, roughness: 0.58, metalness: 0.0, envMapIntensity: 0.6 })
    const woodDark = new THREE.MeshStandardMaterial({ color: 0x3a2412, roughness: 0.62, metalness: 0.0, envMapIntensity: 0.5 })
    const brassMat = new THREE.MeshStandardMaterial({ color: 0xc9a24b, roughness: 0.28, metalness: 0.95, envMapIntensity: 1.0 })
    const brassDk = new THREE.MeshStandardMaterial({ color: 0x9c7a33, roughness: 0.4, metalness: 0.9, envMapIntensity: 0.9 })
    const steelMat = new THREE.MeshStandardMaterial({ color: 0xb8bcc2, roughness: 0.3, metalness: 0.9, envMapIntensity: 1.0 })
    const bakeMat = new THREE.MeshStandardMaterial({ color: 0x14110d, roughness: 0.38, metalness: 0.3, envMapIntensity: 0.7 })
    const panelMat = new THREE.MeshStandardMaterial({ color: 0x1a1610, roughness: 0.5, metalness: 0.35, envMapIntensity: 0.6 })
    const darkMat = new THREE.MeshStandardMaterial({ color: 0x09070a, roughness: 0.85, metalness: 0.1 })
    const cableMat = new THREE.MeshStandardMaterial({ color: 0x161616, roughness: 0.75, metalness: 0.05 })    

    const TOP = 1.1

    // ── 케이스 ──
    const body = new THREE.Mesh(new RoundedBoxGeometry(12.5, 3.2, 10.6, 5, 0.3), woodMat)
    body.position.set(0, -0.5, 0.2)
    scene.add(body)
    const base = new THREE.Mesh(new RoundedBoxGeometry(13.3, 0.7, 11.4, 4, 0.2), woodDark)
    base.position.set(0, -2.45, 0.2)
    scene.add(base)

    // 키보드+램프보드용 검은 패널
    const topPanel = new THREE.Mesh(new RoundedBoxGeometry(11.4, 0.14, 7.4, 3, 0.08), panelMat)
    topPanel.position.set(0, TOP + 0.02, 1.5)
    scene.add(topPanel)

    // 뒤쪽 로터 하우징 (살짝 솟음, 얕게)
    const housing = new THREE.Mesh(new RoundedBoxGeometry(12.5, 1.7, 3.0, 4, 0.18), woodMat)
    housing.position.set(0, 1.1, -3.3)
    scene.add(housing)
    const trough = new THREE.Mesh(new THREE.BoxGeometry(9.0, 0.6, 1.85), darkMat)
    ...
    // ── 앞면 플러그보드 ──
    const pbPanel = new THREE.Mesh(new RoundedBoxGeometry(11.0, 2.6, 0.25, 3, 0.08), bakeMat)
    ...
    function buildCables(plugStr: string) {
      ...
      for (let i = 0; i + 1 < s.length; i += 2) {
        const a = socketPos[s[i]], b = socketPos[s[i + 1]]
        if (!a || !b || s[i] === s[i + 1]) continue
        if (seen.has(s[i]) || seen.has(s[i + 1])) continue
        seen.add(s[i]); seen.add(s[i + 1])
        ...
        const tube = new THREE.Mesh(new THREE.TubeGeometry(curve, 46, 0.055, 8, false), cableMat)
        cableGroup.add(tube)
      }
    }
    ...
    ROWS.forEach((row, r) => {
      ...
      chars.forEach((ch, i) => {
        const x = (i - (chars.length - 1) / 2) * DX
        const z = LAMPZ[r]
        const holder = new THREE.Mesh(lampHoleGeo, new THREE.MeshStandardMaterial({ color: 0x1a1712, roughness: 0.6, metalness: 0.2 }))
        holder.position.set(x, TOP + 0.05, z)
        scene.add(holder)
        const mat = new THREE.MeshStandardMaterial({ color: 0x2a2417, emissive: 0xffce4d, emissiveIntensity: 0, roughness: 0.5, metalness: 0.2 })
    ...
    for (let r = 0; r < 3; r++) {
      ...
      scene.add(spin)
      const disc = new THREE.Mesh(new THREE.CylinderGeometry(0.80, 0.80, 0.86, 56), steelMat)
      disc.rotation.z = Math.PI / 2
      spin.add(disc)
      const tyre = new THREE.Mesh(new THREE.CylinderGeometry(0.83, 0.83, 0.46, 56), bakeMat)
      tyre.rotation.z = Math.PI / 2
      spin.add(tyre)
      for (let i = 0; i < 26; i++) {
        const a = (i / 26) * Math.PI * 2
        const ch = String.fromCharCode(65 + i)
        const lm = new THREE.MeshBasicMaterial({ map: letterTex(ch, "#efe7d2"), transparent: true, depthWrite: false })
        ...
        gl.quaternion.setFromRotationMatrix(new THREE.Matrix4().makeBasis(new THREE.Vector3(1, 0, 0), yA, zA))
      ...
      for (const ex of [-0.28, 0.28]) {
        for (let k = 0; k < grooves; k++) {
          ...
          const gv = new THREE.Mesh(new THREE.BoxGeometry(0.1, 0.04, 0.05), brassDk)
      ...
      for (const ex of [-0.43, 0.43]) {
        const cr = new THREE.Mesh(new THREE.TorusGeometry(0.8, 0.05, 10, 40), brassMat)
    ...
    for (let r = 0; r < 3; r++) {
      ...
      for (const dz of [-0.2, 0.2]) {
        const b = new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.04, 0.05), brassMat)
        b.position.set(rotorX[r], wy, rotorZ + dz)
        scene.add(b)
      }
      for (const dx of [-0.2, 0.2]) {
        const b = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.04, 0.36), brassMat)
    ...
    scene.add(reflGrp)
    const reflDrum = new THREE.Mesh(new THREE.CylinderGeometry(0.62, 0.62, 0.42, 48), bakeMat)
    reflDrum.rotation.z = Math.PI / 2
    reflGrp.add(reflDrum)
    const reflHub = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 0.18, 24), brassMat)
    reflHub.rotation.z = Math.PI / 2
    reflHub.position.x = -0.28
    reflGrp.add(reflHub)
    const reflCap = new THREE.Mesh(new THREE.SphereGeometry(0.2, 20, 16), brassMat)
    reflCap.position.x = -0.37
    reflGrp.add(reflCap)
    for (let i = 0; i < 26; i++) {
      const a = (i / 26) * Math.PI * 2
      const pin = new THREE.Mesh(new THREE.CylinderGeometry(0.035, 0.035, 0.1, 10), brassMat)
      pin.rotation.z = Math.PI / 2
      pin.position.set(0.24, Math.cos(a) * 0.45, Math.sin(a) * 0.45)
      reflGrp.add(pin)
    }
    // ── 엔트리 휠 ──
    const etwMat = new THREE.MeshStandardMaterial({ color: 0x161208, roughness: 0.5, metalness: 0.3, envMapIntensity: 0.7 })
    const etwGrp = new THREE.Group()
    etwGrp.position.set(1.78, rotorY, rotorZ)
    scene.add(etwGrp)
    const etwDrum = new THREE.Mesh(new THREE.CylinderGeometry(0.62, 0.62, 0.4, 48), etwMat)
    etwDrum.rotation.z = Math.PI / 2
    etwGrp.add(etwDrum)
    const etwHub = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 0.18, 24), brassMat)
    etwHub.rotation.z = Math.PI / 2
    etwHub.position.x = 0.27
    etwGrp.add(etwHub)
    const etwCap = new THREE.Mesh(new THREE.SphereGeometry(0.2, 20, 16), brassMat)
    ...
    scene.add(lidPivot)
    const lid = new THREE.Mesh(new RoundedBoxGeometry(12.5, 0.35, 9.0, 4, 0.18), woodMat)
    ...
    for (const sx of [-3.6, 3.6]) {
      const barrel = new THREE.Mesh(new THREE.CylinderGeometry(0.16, 0.16, 0.9, 20), brassMat)
    ...
    const unsubs = [
      rotorPos.subscribe(updateRotors),
      lamp.subscribe(setLamp),
      lastKey.subscribe((v) => { pressedCh = v }),
      config.subscribe((c) => buildCables(c?.plugs ?? "")),
    ]
...
<style>
  ...
  .viewport :global(canvas) { display: block; }
</style>
